...
def test_trial_workflow(patient_id: int = 41):
    ...
    try:
            ...
            for i, trial in enumerate(trial_result.get("trials", []), 1):
                ...
                if hasattr(trial, "metadata"):
                    ...
                    if "metadata" in trial:
                        ...
                        diseases = trial["metadata"].get("diseases", "N/A")
                        content = (
                            trial.get("page_content", "N/A")[:200]
                            if trial.get("page_content")
                            else "N/A"
                        )
                    else:
                        ...
                        content = (
                            trial.get("content", "N/A")[:200]
                            if trial.get("content")
                            else "N/A"
                        )
        ...
        if state.get("trials"):
            ...
            if grade_result.get("relevant_trials"):
                ...
                for i, trial in enumerate(grade_result.get("relevant_trials", []), 1):
                    ...
                    print(
                        f"      Explanation: {trial.get('explanation', 'N/A')[:150]}..."
                    )
                    ...
                    if further_info:
                        ...
                        print("      Further Information: None")
                    print()
                ...
                print(
                    f"      - Total trials evaluated: {len(grade_result.get('relevant_trials', []))}"
                )
            else:
        ...
        print(f"   - Trials found: {len(trial_result.get('trials', []))}")
        print(
            f"   - Relevant trials: {len(grade_result.get('relevant_trials', [])) if 'grade_result' in locals() else 0}"
        )
        print(
            f"   - Trials graded as 'Yes': {yes_count if 'yes_count' in locals() else 0}"
        )
        print(
            f"   - Trials graded as 'No': {no_count if 'no_count' in locals() else 0}"
        )
        print(
            f"   - Trial found: {grade_result.get('trial_found', 'N/A') if 'grade_result' in locals() else 'N/A'}"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Starting trial-focused workflow testing for Patient ID: {args.patient_id}..."
    )
