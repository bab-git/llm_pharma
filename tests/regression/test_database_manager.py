...
def test_database_manager():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if patient_data:
                print(
                    f"✅ Retrieved patient data: {patient_data['name']}, age {patient_data['age']}"
                )
            else:
        ...
        try:
            ...
            original_policy_path = os.path.join(
                original_project_root, "source_data", "instut_trials_policy.md"
            )

            if os.path.exists(original_policy_path):
                # Copy policy file to temp directory
                temp_policy_path = os.path.join(
                    temp_dir, "source_data", "instut_trials_policy.md"
                )
                ...
                print(
                    f"✅ Created policy vector store with {policy_store._collection.count()} documents"
                )
            else:
        ...
        try:
            ...
            if os.path.exists(csv_path):
                ...
                if trial_store:
                    print(
                        f"✅ Created trial vector store with {trial_store._collection.count()} trials"
                    )
                else:
