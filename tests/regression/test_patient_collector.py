...
def test_create_demo_patient_database(temp_db_path):
    ...
    for patient_id in range(1, 6):
        ...
        assert (
            "medical_history" in patient_data
        ), f"Patient {patient_id} should have medical history"
...
def test_patient_collector_node_integration(patient_collector_config):
    ...
    for prompt, expected_id in test_cases:
        ...
        try:
            ...
            assert (
                result["patient_id"] == expected_id
            ), f"Should extract ID {expected_id} from '{prompt}'"

            # Verify required keys exist
            assert (
                "patient_data" in result
            ), f"Should have patient_data key for patient {expected_id}"
            assert (
                "patient_profile" in result
            ), f"Should have patient_profile key for patient {expected_id}"
            assert (
                "policy_eligible" in result
            ), f"Should have policy_eligible key for patient {expected_id}"

            # Verify patient data exists (for patients 1-5)
            if expected_id <= 5:
                assert (
                    result["patient_data"] is not None
                ), f"Should have data for patient {expected_id}"
                assert (
                    len(result["patient_profile"]) > 0
                ), f"Profile should not be empty for patient {expected_id}"

            print(
                f"    ✓ Passed: ID {result['patient_id']}, Profile length: {len(result.get('patient_profile', ''))}"
            )
...
def test_patient_collector_with_non_existent_patient(patient_collector_config):
    ...
    try:
        ...
        assert (
            result["patient_id"] == 99
        ), "Should extract ID even for non-existent patient"

        # Should have empty patient data
        assert (
            result["patient_data"] == {}
        ), "Should have empty data for non-existent patient"

        # Should have empty profile
        assert (
            result["patient_profile"] == ""
        ), "Should have empty profile for non-existent patient"
...
def test_initialize_patient_collector_system(temp_db_path):
    ...
    config = initialize_patient_collector_system(
        use_free_model=True, db_path=temp_db_path, force_recreate_db=True
    )

    # Verify config
    assert isinstance(
        config, PatientCollectorConfig
    ), "Should return PatientCollectorConfig"
...
def run_all_tests():
    ...
    try:
        ...
        test_get_patient_data(
            PatientCollectorConfig(use_free_model=True, db_path=temp_db_path)
        )
